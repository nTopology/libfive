...
SimplexDCIntersecter<N>::~SimplexDCIntersecter() {
    ...
    if constexpr (N == 3) {
        auto sortedVerts = m.verts; // For debugging, remove
        sortedVerts.erase(sortedVerts.begin());
        auto cmpVerts = [](Eigen::Vector3f a, Eigen::Vector3f b) {
            return std::array{ a.x(), a.y(), a.z() } < std::array{ b.x(), b.y(), b.z() };
        };
        std::sort(sortedVerts.begin(), sortedVerts.end(), cmpVerts);
        for (auto iter = sortedVerts.begin(); iter != sortedVerts.end(); ++iter) {
            auto iter2 = iter + 1;
            assert(iter2 == sortedVerts.end() || *iter2 != *iter);
        }
    }
}
...
void SimplexDCIntersecter<N>::load(const std::array<Input*, 1 << (N - 2)>& ts)
{
    if constexpr (N == 3) {
        ...
        for (auto cell = 0; cell < 2; ++cell) {
            ...
            if (!isSame) {
                intersection->sourceDimensions = { 2, 3 };
            }
            ...
            ts[cell]->leaf->intersecterCirculatorRecords23[Axis::toIndex(A) * 2 + cell] =
                std::make_shared< SimplexDCCirculator<N, false>>(circulator);
        }
    }
    else {
...
void SimplexDCIntersecter<N>::load(const std::array<Input*, 1 << (N - 1)>& ts)
{
    ...
    if constexpr (N == 3) {
        ...
        for (auto faceAxisIdx = 0; faceAxisIdx < 2; ++faceAxisIdx) {
            ...
            for (auto facePosition = 0; facePosition < 2; ++facePosition) {
                ...
                if (!isSame) {
                    intersection->sourceDimensions = { 1, 2 };
                    intersection->dup12Record = std::make_shared<SimplexDCIntersection<N>::DupRecord>();
                    for (auto i = 0; i < ts.size(); ++i) {
                        intersection->dup12Record->trees[i] = ts[i];
                    }
                    intersection->dup12Record->idxes = Axis::toIndex(A) * 4 + faceAxisIdx * 2 + facePosition;
                }
                auto circulator = [&]() {
                    if (faceIndex == index) {
                        auto insideIdx =
                ...
                ts[faceIndex]->leaf->intersecterCirculatorRecords12[
                    Axis::toIndex(A) * 8 + faceAxisIdx * 4 + facePosition * 2 + indexFromFace] =
                    std::make_shared<SimplexDCCirculator<N, false>>(circulator);
            }
        }
    }
    ...
    for (auto cell = 0; cell < ts.size(); ++cell) {
        ...
        auto intersection = isSame ? &DCSimplex<N>::dupVertIntersection :
            searchEdge(inside, outside, ts[cell]->leaf->tape);
        if (!isSame) {
            intersection->sourceDimensions = { 1, 3 };
        }
        ...
        ts[cell]->leaf->intersecterCirculatorRecords13[Axis::toIndex(A) * 4 + cell] =
            std::make_shared<SimplexDCCirculator<N, false>>(circulator);
    }
}
...
void SimplexDCIntersecter<N>::load(const std::array<Input*, 1 << N>& ts)
{
    ...
    for (auto edgeAxisIdx = 0; edgeAxisIdx < N; ++edgeAxisIdx) {
        ...
        for (auto edgePosition = 0; edgePosition < 2; ++edgePosition) {
            ...
            if (!isSame) {
                intersection->sourceDimensions = { 0, 1 };
            }
            ...
            ts[edgeIndex]->leaf->intersecterCirculatorRecords01[edgeAxisIdx * 8 + edgeIndex] =
                std::make_shared<SimplexDCCirculator<N, false>>(circulator);
        }
    }
        ...
        for (auto faceAxisIdx = 0; faceAxisIdx < N; ++faceAxisIdx) {
            ...
            for (auto facePosition = 0; facePosition < 4; ++facePosition) {
                ...
                auto intersection = isSame ? &DCSimplex<N>::dupVertIntersection :
                    searchEdge(inside, outside, ts[faceIndex]->leaf->tape);
                if (!isSame) {
                    intersection->sourceDimensions = { 0, 2 };
                    intersection->dup02Record = std::make_shared<SimplexDCIntersection<N>::DupRecord>();
                    for (auto i = 0; i < ts.size(); ++i) {
                        intersection->dup02Record->trees[i] = ts[i];
                    }
                    intersection->dup02Record->idxes = faceAxisIdx * 4 + facePosition;
                }
                ...
                ts[faceIndex]->leaf->intersecterCirculatorRecords02[faceAxisIdx * 8 + faceIndex] =
                    std::make_shared<SimplexDCCirculator<N, false>>(circulator);
            }
        }
    }
    ...
    for (auto cell = 0; cell < ts.size(); ++cell) {
        ...
        auto intersection = isSame ? &DCSimplex<N>::dupVertIntersection :
            searchEdge(inside, outside, ts[cell]->leaf->tape);
        if (!isSame) {
            intersection->sourceDimensions = { 0, 3 };
        }
        ...
        ts[cell]->leaf->intersecterCirculatorRecords03[cell] =
            std::make_shared<SimplexDCCirculator<N, false>>(circulator);
    }
}
    ...
    const std::shared_ptr<Tape>& tape)
{
    ...
    auto pt = intersection->normalized_mass_point()
        .head<N>().template cast<float>().eval();

    std::atomic<uint64_t>* toWrite = nullptr;
    constexpr auto placeholderVal = std::numeric_limits<uint64_t>::max();
    for (auto endSub : { inside, outside }) {
        if (pt == endSub->vert.transpose().template cast<float>().eval()) {
            auto& indexAtomic = endSub->index;
            auto currentIndex = indexAtomic.load();
            if (currentIndex == 0 && indexAtomic.compare_exchange_strong(
                currentIndex, placeholderVal, std::memory_order_relaxed)) {
                toWrite = &indexAtomic;
            }
            else {
                while (currentIndex == placeholderVal) {
                    // Two threads are trying to handle this for the same sub 
                    // at the exact same time, we should be able to just 
                    // spinlock past this rare situation.
                    currentIndex = indexAtomic.load();
                }
    ...
    assert(intersection->index != placeholderVal);
    if (intersection->index == 0) {
        intersection->index = m.pushVertex(pt);
        auto [iter, success] = sources.insert({ pt, {inside->vert, outside->vert} });
        assert(success);
        if (toWrite) {
            toWrite->store(intersection->index);
        }
    }
    else {
        assert(!toWrite);
