...
{
    record = std::make_shared<SimplexDCCirculator<3, true>>(*this);
    if (!forwardRecord) {
        forwardRecord = std::make_shared<std::vector<SimplexDCCirculator<3, true>*>>();
    }
    forwardRecord->push_back(record.get());
    if (chainSize() > 60) {
        auto myForwardRecord = *forwardRecord;
        record = nullptr;
    }
    if (hitError) {
        return;
