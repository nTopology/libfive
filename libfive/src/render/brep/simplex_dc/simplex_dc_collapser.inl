...
SimplexDCCollapser<N, K>::SimplexDCCollapser(
    PerThreadOutput& m, Tree t, double boundCutoff)
    : eval(new Evaluator(t)), owned(true), bound_cutoff(boundCutoff)
{
    // Nothing to do here
}
...
template <unsigned N, unsigned K>
SimplexDCCollapser<N, K>::SimplexDCCollapser(
    PerThreadOutput& m, Evaluator* es, double boundCutoff)
    : eval(es), owned(false), bound_cutoff(boundCutoff)
{
    // Nothing to do here
}
...
void SimplexDCCollapser<N, K>::load(const std::array<Input*, 1 << (N - 2)>& ts)
{
    ...
    if (pos != 8) {
        ...
        auto recalc = [&]() {
            SimplexDCRecalculator<N, 2> recalculator(
              eval, cell, faceSubIndex, bound_cutoff);
            faceSub->vert = recalculator.getNewVertex();
            auto type = cell->type;
            if (N == 3 && type == Interval::AMBIGUOUS) {
                // If either side of the face is non-ambiguous, our sub
                // must match it.  In the future, we may want to perform sub
                // collapsing before setting the cell type (unless the cell
                // type is set based on interval evaluation).
                type = ts[1 - index]->type;
            }
            ...
            case Interval::EMPTY:
                faceSub->inside = (type == Interval::FILLED);
                break;
            default:
                assert(false);
        ...
        if (relativeNeighbor.dimension() == 0) 
        {
            ...
            if (intermediatesResults[0] == 2) {
                ...
                // corner, or use the recalculator.
                for (auto i = 0; i < 2; ++i) {
