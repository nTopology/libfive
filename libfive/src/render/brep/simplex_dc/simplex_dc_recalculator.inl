...
SimplexDCRecalculator<N, K>::SimplexDCRecalculator(
    Evaluator* eval, SimplexDCTree<N>* tree, int targetSub)
...
Eigen::Matrix<double, N, 1> SimplexDCRecalculator<N, K>::getNewVertex()
{
    ...
    if (is_ambiguous) {
        ...
        while (toContinue) {
            ...
            region = nextRegion;
            current_vertex = summed_qef.solveBounded(region, 0.).position;
        }
    }
    if constexpr (N == K) {
    ...
    SimplexDCTree<N>* tree, int targetSub)
{
    ...
    if (tree->isBranch()) {
        for (auto i = 0; i < ipow(3, N); ++i) {
            ...
            auto floating = childIndex.floating();
            if (floating | targetNeighbor.floating() != ipow(3, N) - 1) {
                // There is a dimension in which targetNeighbor is fixed,
                // and our child index is fixed (meaning we want it to be
                // on one side or the other); it can't be on one side or
                // the other, so we skip this one.
                continue;
            }

            auto childPos = childIndex.pos();
            ...
            auto newPos = targetNeighbor.floating() | targetNeighbor.pos();
            auto newFloat = targetNeighbor.floating() & ~floating;

            auto newSub = NeighborIndex::fromPosAndFloating(newPos, newFloat);
            addBorderSubs(tree->child(childPos), newSub.i);
        }
    }
    else {
