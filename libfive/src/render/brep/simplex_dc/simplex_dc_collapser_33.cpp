...
void SimplexDCCollapser<3, 3>::load(Input* t)
{
    ...
    auto centerCell = [&]()
    {
        ...
        // (or are split), so we have to simply use the center.
        cellSub->vert = region.center();
        switch (t->type) {
    ...
    if (targetDimension < 3) {
        ...
        auto checkIntermediates = [&](auto intermediates) {
            ...
            if constexpr (size == 6) { // Debugging, remove
                t->leaf->cellIntermediates = intermediates;
                t->leaf->cellIntermediatesUncollapsing = uncollapsing;
                t->leaf->intermediatesDisqualify = intermediatesDisqualify;
            }
        };
