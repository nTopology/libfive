...
DCSimplex<2>* SimplexDCCirculator<2, true>::forwardSimplex(
    ...
    bool allowDegenerate, std::array<int, 2>& dims)
{
    ...
    if (!allowDegenerate && thirdSub == insideSub) {
        return forwardSimplex(cell, thirdIdx, outsideIdx, false, dims);
    }
    else if (!allowDegenerate && thirdSub == outsideSub) {
        return forwardSimplex(cell, insideIdx, thirdIdx, false, dims);
    }
    else {
        assert(cell->leaf);
        ...
        dims = { int(insideDim), int(outsideDim) };
        return &simplex;
    }
    ...
    const SimplexDCIntersection<2>* intersection) const
{
    ...
    [[maybe_unused]] auto [res, success] =
        simplex()->insertIntersection(
            dims[index][0], dims[index][1], intersection);
    assert(success);
