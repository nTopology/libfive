...
SimplexQEF<N>::SimplexQEF(Eigen::Matrix<double, N, N + 1>&& vertices, 
    ...
    mass_point(Eigen::Matrix<double, N + 1, 1>::Zero()), cutoff_rate(cutoffRate)
{
...
//            assert(val != perp.dot(paddedVerts.col(i)));
            ...
            // only up to a constant positive factor.  So just set it to 1 (as
            // that way we will store a -1 in the constraint, so that our single
            // point passes the constraint).
            val = 1.;
        }
...
const SimplexQEF<N - bitcount(mask)>& SimplexQEF<N>::sub() const
{
    ...
    if constexpr (mask == 0) {
        ...
        if (!out.has_value()) {
            ...
            out.emplace(std::move(newVerts), 0, cutoff_rate);
...
typename SimplexQEF<N>::PointOpt SimplexQEF<N>::solveFromMask(
    ...
    unsigned maxCutoffLimit /*changed by function*/) const
{
    PointOpt myBest(std::nullopt);
    if ((1 << current_mask) & currentMasks) {
        for (auto myCutoffLimit = 0; myCutoffLimit <= maxCutoffLimit; 
                ++myCutoffLimit) {
            auto rawResult = sub<current_mask>().solveRaw(myCutoffLimit);
            auto subFailures = 
                sub<current_mask>().constraintFailures(rawResult);
            assert(bitcount(subFailures) + bitcount(current_mask) <= N);
            assert(myCutoffLimit >= N - bitcount(current_mask));
            if (myCutoffLimit == N - bitcount(current_mask)) {
                for (unsigned i = 0, subI = 0; i <= N; ++i) {
                    assert(i >= subI);
                    assert(i <= subI + bitcount(current_mask));
                    auto shifted = 1 << i;
                    if (shifted & current_mask) {
                        // Don't check the constraints that 
            ...
            if (subFailures == 0) {
                ...
                if (!std::isfinite(result.x()) || !std::isfinite(result.y()) ||
                    ...
                    // This should not happen, but between floating-point error
                    // and nearly degenerate simplices is not entirely 
                    // impossible; if it happens, just ignore that result and
                    // go with the mass point if necessary.
                    assert(false);
                    continue;
                }
                if (myCutoffLimit == 0) {
                    ...
                    maxCutoffLimit = myCutoffLimit - 1;
                    myBest = result;
                    break;
                }
            }
    ...
    if constexpr (bitcount(next_mask) > N) {
        ...
        auto nextResult = solveFromMask<next_mask>(
...
inline typename SimplexQEF<N>::Point SimplexQEF<N>::solve() const
{
    ...
    auto maskDimensionality = N;
    while (currentMasks != 0) {
        for (auto i = 0; i < 8 * sizeof(unsigned); ++i) {
            if (1 << i & currentMasks) {
                assert(bitcount(i) == N - maskDimensionality);
                if (i != 0) {
...
template<unsigned N>
void SimplexQEF<N>::checkVertConstraints()
{
    for (auto i = 0; i < N + 1; ++i) {
        auto failures = constraintFailures(paddedVerts.col(i));
//        assert(!(failures & (1 << i)));
    }
}
...
SimplexQEF<N>::solveRaw(unsigned cutoffLimit) const
{
    ...
    auto highestEigenvalue = eigenvalues.maxCoeff();

    // Simplex DC meshing is fairly robust to "noisy" inputs, so we don't
    // usually need much of an eigenvalue cutoff.  However, there is still
    // somewhat of an effect, so we typically want to try to handle "escaping"
    // results (those that would be outside the simplex) via eigenvalue cutoffs
    // before going to lower-dimensional sub-simplices.

    auto eigenvalueCutoff = cutoff_rate * highestEigenvalue;

    assert(cutoffLimit <= N);
    if (cutoffLimit < N) {
      if (cutoffLimit == 0) {
        eigenvalueCutoff = 0;
      }
      else {
        std::array<double, N> absValues;
        for (unsigned i = 0; i < N; ++i) {
          absValues[i] = fabs(eigenvalues[i]);
        }
        std::sort(absValues.begin(), absValues.end());
        eigenvalueCutoff = std::max(eigenvalueCutoff, absValues[cutoffLimit]);
      }
    }


    for (unsigned i = 0; i < N; ++i) {
        if (std::abs(eigenvalues[i]) > highestEigenvalue * 1e-15)
        {
