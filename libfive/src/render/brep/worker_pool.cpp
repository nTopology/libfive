...
template <typename T, typename Neighbors, unsigned N>
void runTbb(typename WorkerPool<T, Neighbors, N>::Task& task,
    ...
    if (can_subdivide)
    {
        ...
        if (t->type == Interval::AMBIGUOUS)
        {
            ...
            auto recurse = [&](Task task) {
              runTbb<T, Neighbors, N>(task, tlEvs, tlPools, settings, level + 1);
            };
...
template<typename T, typename Neighbors, unsigned N>
Root<T> WorkerPool<T, Neighbors, N>::buildTbb(Tree                t,
                                              const Region<N>&    region_,
                                              const BRepSettings& settings)
{
  tbb::enumerable_thread_specific<Evaluator> tlEvs(t);
  tbb::enumerable_thread_specific<typename T::Pool> tlPools;
  ...
  Evaluator& ev = tlEvs.local();
  Task task({root, ev.getDeck()->tape, Neighbors(), settings.vol});

  runTbb<T, Neighbors, N>(task, tlEvs, tlPools, settings, 0);
  ...
  //nTopology::ntlog::info("main", "{0} pools, {1} roots", tlPools.size(), out.size());
