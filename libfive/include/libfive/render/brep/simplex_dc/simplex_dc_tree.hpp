...
struct SimplexDCIntersection : Intersection<N>
{
    ...
    mutable std::array<int, 2> sourceDimensions = { -1, -1 };// For debugging, remove
    struct DupRecord; // For debugging, remove.

    mutable std::shared_ptr<DupRecord> dup02Record;
    mutable std::shared_ptr<DupRecord> dup12Record;
};
...
struct DCSimplex
{
    ...
    std::shared_ptr<DupRecord> dup01Record;
...
class SimplexDCCirculator; // Here for debugging, remove.
...
struct SimplexDCLeaf
{
    ...
    std::array<NeighborIndex, 6> cellIntermediates; // For debugging, remove.
    std::array<bool, 6> cellIntermediatesUncollapsing; // For debugging, remove.
    std::array<bool, 6> intermediatesDisqualify; // For debugging, remove.
     ...
     *  instead.  Sets *trueIndex, if not null to the index of that subspace.*/
    SimplexLeafSubspace<N>* collapsedSub(
        unsigned index, unsigned* trueIndex = nullptr);
    ...
    constexpr static double qefShrink = 1.;

    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 24> intersecterCirculatorRecords01;
    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 24> intersecterCirculatorRecords02;
    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 8> intersecterCirculatorRecords03;
    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 24> intersecterCirculatorRecords12;
    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 12> intersecterCirculatorRecords13;
    std::array<std::shared_ptr<SimplexDCCirculator<N, false>>, 6> intersecterCirculatorRecords23;
};

template <unsigned N>
using SimplexDCTree = SimplexTree<N, SimplexDCLeaf<N>>;

template <unsigned N> struct DCSimplex<N>::DupRecord 
{
    std::array<const SimplexDCTree<N>*, 8> trees;
    int idxes;
};

template <unsigned N> struct SimplexDCIntersection<N>::DupRecord
{
    std::array<const SimplexDCTree<N>*, 8> trees;
    int idxes;
};
