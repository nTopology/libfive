...
#include <memory>  // for debugging, remove
...
public:
    ...
    std::shared_ptr<SimplexDCCirculator<3, true>> record;  // for debugging, remove
    std::shared_ptr<std::vector<SimplexDCCirculator<3, true>*>> forwardRecord; // for debugging, remove 
    mutable const SimplexDCIntersection<3>* storedIntersection; // for debugging, remove
    mutable const SimplexDCMinEdge<3>* calcedEdge; // for debugging, remove
    mutable const DCSimplex<3>* calcedSimplex; // for debugging, remove
    int chainSize() {
        if (record) {
            return record->chainSize() + 1;
        }
        else {
            return 1;
        }
...
protected:
    ...
    DCSimplex<3>* mutableSimplex() const {
        ...
        calcedEdge = std::get<SimplexDCMinEdge<3>*>(edge);
        calcedSimplex = &std::get<SimplexDCMinEdge<3>*>(edge)->
