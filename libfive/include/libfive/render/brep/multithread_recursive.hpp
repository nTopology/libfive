...
template <class Divisible, class Out, class Local, class PreCallable, class PostCallable>
Out multithreadRecursive(Divisible                               root,
                         ...
                         unsigned                                childNo = -1)
{
  struct Task
  {
    ...
    void operator()() const { multithreadRecursive(root, locals, pre, post, parent, childNo); }
  };
  ...
  if constexpr (std::is_same_v<Out, void>) {
    ...
    if (toRecurse) {
      ...
      for (auto& child : children) {
        ...
        auto task = Task(child, locals, pre, post);
        list.run(task);
      }
      list.wait();
    ...
    if (toRecurse) {
      ...
      auto            i = 0;
      for (auto& child : children) {
        auto task = Task(child, locals, pre, post, &out, i++);
        list.run(task);
      }
