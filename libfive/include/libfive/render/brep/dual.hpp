...

{
    ...
    while (!done.load() && !settings.cancel.load())
    {
        ...
        // Handle the cell itself, since if it is a leaf and some walkers need
        // to handle individual cells (specialized for N = 2 or 3)
        v.load(t);
        ...
        {
            // Do the actual work for faces, edges, and corners 
            // (also specialized for N = 2 or 3)
            Dual<N>::work(t, v);
