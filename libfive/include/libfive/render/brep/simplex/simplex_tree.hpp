...
public:
  ...
  using Pool = typename Leaf::ParentPool;
     ...
     *
     *  Normally, we would use ipow(3, N) as the size parameter of the array.
     *  However, in some versions of gcc, constexpr functions are treated as
     *  having internal linkage, to the point that using ipow in our argument
     *  would give this method internal linkage.  Since this method needs
     *  external linkage due to being defined in simplex_tree.inl and used in
     *  the simplex DC collapser, we use a ternary operator instead.
     */
