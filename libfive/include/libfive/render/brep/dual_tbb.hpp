            ...
            std::function<M(PerThreadBRep<N>&, int)> MesherFactory)
{
    ...
    tbb::enumerable_thread_specific<PerThreadBRep<N>> breps(global_index);
    tbb::enumerable_thread_specific<M> ms([&]() {
        return MesherFactory(breps.local(), 0)
    });

    if (settings.progress_handler) {
        settings.progress_handler->nextPhase(settings.workers + 1);
    }

    run(m, t.get(), settings);

    // Handle the top tree edges (only used for simplex meshing)
    if (M::needsTopEdges()) {
        auto m = MesherFactory(breps, 0);
...
template <unsigned N>
template <typename T, typename V>
void Dual<N>::run(T* task,
                  tbb::enumerable_thread_specific<V>& ms
                  const BRepSettings& settings)
