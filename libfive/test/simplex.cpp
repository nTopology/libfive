...
TEST_CASE("SimplexPool")
{
    ...
    settings.workers = 1;
    auto t = Root<SimplexTree<2>>::build(c, r, settings);
...
TEST_CASE("SimplexTree<2>: cell collapsing")
{
    SECTION("Rotated box") {
        ...
        settings.workers = 1;
        auto t = Root<SimplexTree<2>>::build(shape, r, settings);
        REQUIRE(!t->isBranch());
    }
}
...
TEST_CASE("SimplexTree<3>: types")
{
    ...
    auto t = Root<SimplexTree<3>>::build(c, r, BRepSettings());
    REQUIRE(t.get() != nullptr);
...
TEST_CASE("SimplexTree<3>: Corner positions")
{
    SECTION("Single-cell sphere")
    {
        ...
        auto t = Root<SimplexTree<3>>::build(c, r, settings);
...
TEST_CASE("SimplexMesher<3>: box with problematic edges")
{
    ...
    settings.max_err = -1;
    auto t = Root<SimplexTree<3>>::build(shape, r, settings);
    CHECK_CORNER_POSITIONS(t.get(), r);
...
TEST_CASE("SimplexMesher<3>: tricky shape")
{
    ...
    auto t = Root<SimplexTree<3>>::build(b, r, settings);
    t->assignIndices(settings);
...
TEST_CASE("SimplexMesher<3>: sphere")
{
    ...
    settings.workers = 1;
    auto t = Root<SimplexTree<3>>::build(s, r, settings);
    t->assignIndices(settings);
...
TEST_CASE("SimplexMesher: menger sponge")
{
    ...
    BRepSettings settings;
    settings.min_feature = 0.1;
    auto t = Root<SimplexTree<3>>::build(sponge, r, settings);
    t->assignIndices(settings);
